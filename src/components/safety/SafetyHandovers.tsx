...
export default function SafetyHandovers() {
    return (
        <section id="verification" className="bg-[#FBFAF7] px-6 py-24 lg:px-8">
            <div className="mx-auto max-w-5xl">
                <div className="mx-auto max-w-3xl text-center">
                    <div className="flex items-center justify-center gap-3 text-sm font-bold uppercase tracking-[0.15em] text-[#1FA463]">
                        <span className="h-[2px] w-5 bg-[#1FA463]"></span>
                        SECURE HANDOVERS
                    </div>
                    <h2 className="mt-6 text-4xl font-extrabold text-[#0B3D2E] sm:text-5xl leading-[1.2]" style={{ fontFamily: "Manrope, sans-serif" }}>
                        Every Pickup. Every Drop-off.<br />Verified.
                    </h2>
                    <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-[#4A574F]">
                ...
                <div className="relative mt-20">
                    ...
                    <div className="absolute top-10 left-[12.5%] right-[12.5%] h-[2px] border-t-[2px] border-dashed border-[#E4E9E3] hidden md:block"></div>
                    
                    <div className="grid grid-cols-1 gap-10 md:grid-cols-4 relative z-10">
                        {processSteps.map((step, index) => (
                            <div key={step.title} className="flex flex-col items-center text-center">
                                <div className="flex h-20 w-20 items-center justify-center rounded-[24px] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E4E9E3]/50">
                                    <div className={`flex items-center justify-center ${step.accent ? "text-[#1D6FE0]" : "text-[#1FA463]"}`}>
                                        {step.title.includes("School") ? <SchoolIcon /> : <SimpleHouseIcon />}
                                    </div>
                                </div>
                                <p className="mt-8 text-[11px] font-extrabold uppercase tracking-[0.15em] text-[#1FA463]">
                                    STEP 0{index + 1}
                                </p>
                                <h3 className="mt-2 text-[15px] font-bold text-[#0B3D2E]">{step.title}</h3>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="mt-16 flex items-center justify-center gap-3 text-sm font-medium text-[#4A574F]">
