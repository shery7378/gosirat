...
export default function SafetyEmergency() {
    return (
        <section id="emergency" className="bg-[#F1F4F1] px-6 py-20 lg:px-8">
            <div className="mx-auto max-w-7xl">
                <div className="mx-auto max-w-3xl text-center">
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#1FA463]">Prepared, not just reactive</p>
                ...
                <div className="mt-12 grid gap-8 md:grid-cols-2 items-stretch">
                    {readinessCards.map((card) => (
                        <div key={card.title} className="rounded-[24px] border border-[#E4E9E3]/80 bg-white p-8 lg:p-10 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col">
                            <div className={`flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-[14px] ${card.accent ? "bg-[#DDE9FC] text-[#1D6FE0]" : "bg-[#DCF3E6] text-[#1FA463]"}`}>
                                {card.accent ? <HeartIcon /> : <ShieldPlusIcon />}
                            </div>
                            <h3 className="mt-6 text-[22px] font-bold text-[#0B3D2E] tracking-tight">{card.title}</h3>
                            <p className="mt-3 text-[15px] leading-7 text-[#4A574F]">{card.text}</p>
                            <ul className="mt-8 flex flex-col">
                                {card.bullets.map((bullet, idx) => (
                                    <li key={bullet} className={`flex items-center gap-4 py-[14px] text-[15px] font-medium text-[#0B3D2E] ${idx !== 0 ? 'border-t border-[#E4E9E3]/70' : ''}`}>
                                        <span className="text-[#1FA463] shrink-0">
                                            <ThinCheckIcon />
                                        </span>
                                        {bullet}
                                    </li>
